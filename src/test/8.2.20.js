...
class Test_8_2_20 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.20";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - claim sub MUST be present";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.sub==null || grantTokenInnerSignedTokenPayload.sub=='') {
                this.notes = grantTokenInnerSignedTokenPayload;
                throw("grant token JWS (Grant Token Inner Signed Token) claim sub is not present");
            }                  
        }
