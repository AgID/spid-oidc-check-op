...
class Test_8_2_19 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.19";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - if present, the value of nbf MUST be a valid unix time";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.nbf!=null && !moment.unix(grantTokenInnerSignedTokenPayload.nbf).isValid()) {
                this.notes = grantTokenInnerSignedTokenPayload.nbf;
                throw("grant token JWS (Grant Token Inner Signed Token) if present, claim nbf is not a valid unix time");
            }          
        }
