...
class Test_8_2_17 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.17";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - the value of exp MUST be a valid unix time";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if (!moment.unix(grantTokenInnerSignedTokenPayload.exp).isValid()) {
                this.notes = grantTokenInnerSignedTokenPayload.exp;
                throw("grant token JWS (Grant Token Inner Signed Token) claim exp is not a valid unix time");
            }            
        }
