...
class Test_8_2_26 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.26";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - The payload MUST contain all the user attributes declared into the parameter aa-required-attributes into the OpenAPI document of the AA";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.jti==null || grantTokenInnerSignedTokenPayload.jti=='') {
                ...
                throw("grant token JWS (Grant Token Inner Signed Token) claim jti is not present");
            }         
        }

        this.notes = "Please check that the payload contains all the user attributes declared into the parameter aa-required-attributes into the OpenAPI document of the AA";
        return false;
    }

}

module.exports = Test_8_2_26