...
class Test_8_2_10 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.10";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - the value of sid MUST be equal to 'oidc:jti' where jti is the value of claim jti of the id_token";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.sid==null || grantTokenInnerSignedTokenPayload.sid=='') {
                this.notes = grantTokenInnerSignedTokenPayload;
                throw("grant token JWS (Grant Token Inner Signed Token) claim sid is not present");
            }         

            if(grantTokenInnerSignedTokenPayload.sid != "oidc:" + id_token_payload.jti) {
                this.notes = grantTokenInnerSignedTokenPayload.sid + " != oidc:" + id_token_payload.jti;
                throw("the value of sid is not equal to 'oidc:" + id_token_payload.jti + "'");
            }
        }
