...
class Test_4_4_13 extends TestUserinfoResponse {
    ...
    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse, userinforequest, userinforesponse) {
        ...
        this.num = "4.4.13";
        this.description = "Userinfo Signed Token Payload: the value of exp MUST be a valid unix time";
    ...
    async exec() {
        ...
        if(userinfo_sig_token_verified.payload.exp==null || userinfo_sig_token_verified.payload.exp=='') {
            this.notes = userinfo_sig_token_verified.payload;
            throw("claim exp is not present");
        }

        if(!moment.unix(userinfo_sig_token_verified.payload.exp).isValid()) {
            this.notes = userinfo_sig_token_verified.payload.exp;
            throw("the value of exp is not a valid unix time");
        }

        this.notes = userinfo_sig_token_verified.payload.exp + " = " + moment.unix(userinfo_sig_token_verified.payload.exp).format('DD/MM/YYYY HH:mm:ss');
