...
class Test_8_2_1 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.1";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - the value of alg MUST be one of ['RS256', 'RS512']";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(jws.header.alg==null || jws.header.alg=='') {
                this.notes = jws.header;
                throw("grant token JWS (Grant Token Inner Signed Token) claim alg is not present");
            }     

            if(!['RS256', 'RS512'].includes(jws.header.alg)) {
                this.notes = jws.header.alg;
                throw("grant token JWS (Grant Token Inner Signed Token) the value of alg is not one of ['RS256', 'RS512']");
            }
        }
