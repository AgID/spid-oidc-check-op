...
class Test_8_2_15 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.15";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - the value of iat MUST be < current date + 3min";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.iat==null || grantTokenInnerSignedTokenPayload.iat=='') {
                this.notes = grantTokenInnerSignedTokenPayload;
                throw("grant token JWS (Grant Token Inner Signed Token) claim iat is not present");
            }       

            if (!moment.unix(grantTokenInnerSignedTokenPayload.iat).isValid()) {
                this.notes = grantTokenInnerSignedTokenPayload.iat;
                throw("grant token JWS (Grant Token Inner Signed Token) claim iat is not a valid unix time");
            }            

            if(!moment.unix(grantTokenInnerSignedTokenPayload.iat).isBefore(moment().add(3, 'm'))) {
                this.notes = grantTokenInnerSignedTokenPayload.iat + " > " + moment().add(3, 'm');
                throw("grant token JWS (Grant Token Inner Signed Token) claim iat is > current date + 3min");
            }               
        }
