...
class Test_4_4_17 extends TestUserinfoResponse {
  constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse, userinforequest, userinforesponse) {
    ...
    this.num = "4.4.17";
    this.description =
      "Userinfo Signed Token Payload: the claims for user attributes requested within the authorization request MUST be present";
  ...
  async exec() {
    ...
    for (c in requested_claims) {
      if (!userinfo_claims.includes(requested_claims[c])) {
        ...
        throw (
          "userinfo response not includes the user attribute claim '" +
          requested_claims[c] +
          "' requested with the authorization request"
        );
      }
    }
