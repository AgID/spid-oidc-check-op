...
class Test_8_2_16 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.16";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - claim exp MUST be present";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.exp==null || grantTokenInnerSignedTokenPayload.exp=='') {
                this.notes = grantTokenInnerSignedTokenPayload;
                throw("grant token JWS (Grant Token Inner Signed Token) claim exp is not present");
            }                  
        }
