...
class Test_8_2_24 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.24";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - claim sub of the act object MUST be equal to the client_id of RP";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.act==null || grantTokenInnerSignedTokenPayload.act=='') {
                ...
                throw("grant token JWS (Grant Token Inner Signed Token) claim act is not present");
            }         
            
            if(!(grantTokenInnerSignedTokenPayload.act===Object(grantTokenInnerSignedTokenPayload.act))) {
                this.notes = grantTokenInnerSignedTokenPayload.act;
                throw("grant token JWS (Grant Token Inner Signed Token) claim act is not a valid json object");                
            }            

            if(grantTokenInnerSignedTokenPayload.act.sub==null || grantTokenInnerSignedTokenPayload.act.sub=='') {
                this.notes = grantTokenInnerSignedTokenPayload.act;
                throw("grant token JWS (Grant Token Inner Signed Token) act object does not contain claim sub");
            }           
            
            if(grantTokenInnerSignedTokenPayload.act.sub!=config_rp.client_id) {
                this.notes = grantTokenInnerSignedTokenPayload.act.sub + " != " + config_rp.client_id;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim sub of the act object is not equal to the client_id of RP: " + config_rp.client_id);
            }            
        }
