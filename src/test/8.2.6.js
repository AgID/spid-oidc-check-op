...
class Test_8_2_6 extends TestTokenResponse {

    constructor(metadata, authrequest, authresponse, tokenrequest, tokenresponse) {
        super(metadata, authrequest, authresponse, tokenrequest, tokenresponse);
        this.num = "8.2.6";
        this.description = "Grant Token JWS (Grant Token Inner Signed Token) - the value of iss MUST be equal to the URL of the OP";
    ...
    async exec() {
        ...
        for(let t in id_token_payload.tokens) {
            ...
            if(grantTokenInnerSignedTokenPayload.iss==null || grantTokenInnerSignedTokenPayload.iss=='') {
                this.notes = grantTokenInnerSignedTokenPayload;
                throw("grant token JWS (Grant Token Inner Signed Token) claim iss is not present");
            }     

            if(grantTokenInnerSignedTokenPayload.iss!=this.metadata.configuration.issuer) {
                this.notes = grantTokenInnerSignedTokenPayload.iss + " != " + this.metadata.configuration.issuer;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iss is not equal to the URL of the OP");
            }         
        }
